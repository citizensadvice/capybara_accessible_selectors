    ...
    context selector.to_s do
      ...
      context "single fieldset" do
        it "selects with an implicit label" do
          ...
          expect(find(:field, "#{label} implicit", fieldset: "Inner circle")).to eq find(:fieldset, "Inner circle").find(:field, "#{label} implicit")
        end

        it "selects with an explicit label" do
          expect(page).to have_selector selector, "#{label} explicit", count: 2
          expect(find(:field, "#{label} explicit", fieldset: "Inner circle")).to eq find(:fieldset, "Inner circle").find(:field, "#{label} explicit")
        end
      end
      ...
      context "two fieldsets" do
        it "selects with an implicit label" do
          ...
          expect(find(:field, "#{label} implicit", fieldset: ["Outer circle", "Inner circle"])).to eq expected
        end

        it "selects with an explicit label" do
          expect(page).to have_selector selector, "#{label} explicit", count: 2
          expected = find(:fieldset, "Outer circle").find(:fieldset, "Inner circle").find(:field, "#{label} explicit")
          expect(find(:field, "#{label} explicit", fieldset: ["Outer circle", "Inner circle"])).to eq expected
        end
      end
    end
  end
