...
module CapybaraAccessibleSelectors
  ...
  module CupriteNodeExtensions
    def accessible_name
      # Match the Selenium path: collapse whitespace so exact-match
      # locators behave consistently across drivers
      Cuprite::AccessibilityComputedValue.resolve(native, "name").to_s.strip.gsub(/\s+/, " ")
    end
  end
end
